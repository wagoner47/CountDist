...
def setup(params_file):
    ...
    Parameters
    ----------
    :param params_file: Location of parameter file with file paths for
    loading and saving data
    :type params_file: `str`
    """
    global SIGMAR_EFF, SIGMAP_RP, DELTA_RLO, DELTA_RLT, DELTA_LN_RPO, \
        DELTA_LN_RPT, DELTA_RPO, MAKE_PLOTS, SHOW_PLOTS
    ...
    if not TYPE in ["DD", "DR", "RD", "RR"]:
        raise ValueError(
            "Invalid paircount type ({}). Choices are DD, DR, RD, RR".format(
                TYPE))
    catalog_meta_data = Table.read(config["run_params"]["ifname"]).meta
    SIGMAR_EFF = catalog_meta_data["SIGMAR"]
    cosmo_config = MyConfigObj(config["fit_params"]["cosmo_file"])
    cosmo = FlatLambdaCDM(
        100.0 * cosmo_config["cosmological_parameters"].as_float("h0"),
        config["cosmological_parameters"].as_float("omega_m"))
    ...
    SIGMAP_RP = CatalogUtils.dr_dz(meta["ZEFF"]) * sigmaz * (
            1.0 + meta["ZEFF"]) / CatalogUtils.dist(meta["ZEFF"])
...
def lnlike(theta, df, col_dict, index_names, func, other_args=None):
    """A general likelihood function that takes the fitting function as
    input. Note this does not have the option for a non-diagonal covariance
    matrix
    ...
    Returns
    -------
    :return: The log-likelihood of the model with the given parameters with
    the data, using the function provided
    :rtype: float
    """
    if other_args is not None:
        f = func(df.index, index_names, *other_args, *theta)
    else:
        f = func(df.index, index_names, *theta)
    return -0.5 * (df[col_dict["m"]] - f).pow(2).div(df[col_dict["var"]]).sum()


def lnprob(theta, df, col_dict, index_names, func, lnprior, other_args=None):
    """A general probability function that takes the fitting function and the
    function for the log-prior as inputs. Note this does not have the option
    for a non-diagonal covariance matrix
    ...
    Returns
    -------
    :return: The log-probability of the model with the given parameters with
    the data, using the function provided, and including the prior
    :rtype: float
    """
    lp = lnprior(theta)
    if not np.isfinite(lp):
        return -np.inf
    return lp + lnlike(theta, df, col_dict, index_names, func, other_args)


def fit_first_pass(init_guess, df, col_dict, index_names, func,
                   other_args=None, state=None):
    """Do a first pass of fitting the function given to the data using
    ...
    arguments. Defualt `None`
    :type other_args: `tuple`
    :param state: The random state to set for the random number generator. If
    `None`, the internal random state is not changed. Default `None`
    :type state: `tuple(str, ndarray of 624 uints, int, int, float)`
    
    Returns
    -------
    :return res.x: The result of using scipy.optimize.minimize to fit the
    ...
    if other_args is not None:
        res = minimize(nll, init_guess, args=other_args)
    else:
        res = minimize(nll, init_guess)
    return res.x


def fit_mcmc(init_guess, df, col_dict, index_names, func, lnprior, nwalkers,
             nsteps, nburnin, nthreads=None, other_args=None, state=None):
    ...
    if state is not None:
        np.random.set_state(state)
    pos = [start + 10. ** (-4 + ndigits(start)) * np.random.randn(ndim) for i in
           range(nwalkers)]
    if threads is None:
...
def r_perp_t_mean(index, index_names, x0, a, alpha):
    """This is the fitting function for the mean of the true perpendicular
    separation with a power law:
    ...
    Returns
    -------
    :return f: The value of the fitting function evaluated on index for the
    given parameters, with the same index as given
...
def get_init_guess_mean_perp(index_names, df, col_dict):
    """Generate an initial guess for the parameters to
    :function:`r_perp_t_mean` for the given index. Also determines the pivot
    points to use for the index or indices being used
    ...
    nx_odd = (nx % 2 == 1)
    if nx_odd:
        x_around_mid = [unique_x[int((nx - 1) / 2) - 1],
                        unique_x[int((nx - 1) / 2) + 1]]
        if twod:
        ...
        if twod:
            a = np.mean(
                [df[col_dict["m"]].loc[(x_around_mid[0], slice(None))].mean(),
                 df[col_dict["m"]].loc[(x_around_mid[1], slice(None))].mean()])
        else:
            a = df[col_dict["m"]].loc[x_around_mid].mean()
    if twod:
        alpha = (df[col_dict["m"]].loc[(x_around_mid[1], slice(None))] -
                 df[col_dict["m"]].loc[
                     (x_around_mid[0], slice(None))]).mean() / (
                        x_around_mid[1] - x_around_mid[0])
    else:
        alpha = (df[col_dict["m"]].loc[x_around_mid[1]] - df[col_dict["m"]].loc[
            x_around_mid[0]]) / (x_around_mid[1] - x_around_mid[0])
    init_guess = np.array([a, alpha])
    return init_guess, x0


def r_perp_t_mean_prior(theta):
    # Flat prior on all
    return 0.0


def fit_mean_perp(index_names, df, col_dict, nwalkers, nsteps,
                  nburnin, nthreads=None, state=None):
    """Wrapper for various fitting steps to be applied to the fitting
    function for the mean of the true perpendicular separation. The fit can
    be done to the difference in perpendicular separations instead if the
    column name dictionary points to columns containing the difference
    statistics instead.
    ...
    init_guess, x0 = get_init_guess_mean_perp(index_names, df, col_dict)
    min_guess = fit_first_pass(init_guess, df, col_dict, index_names,
                               r_perp_t_mean, other_args=(x0,), state=state)
    sampler = fit_mcmc(min_guess, df, col_dict, index_names, r_perp_t_mean,
                       r_perp_t_mean_prior, nwalkers, nsteps, nburnin,
                       nthreads=nthreads, other_args=(x0,),
                       state=np.random.get_state())
