...
static void bind_get_1d_indices(py::module& mod) {
    mod.def(("get_1d_index_from_" + std::to_string(N) + "d").c_str(),
            py::overload_cast<
                    ...
                    const std::array<BinSpecifier, N>&>(&get_1d_indexer_from_nd<
                    N>),
...
void process_auto(NNCountsND<3>& self,
                  const py::array_t<SPosCatalog>& cat,
                  int num_threads) {
    py::gil_scoped_acquire acquire;
...
template<>
void process_auto(NNCountsND<2>& self,
                  const py::array_t<SPosCatalog>& cat,
                  int num_threads) {
    py::gil_scoped_acquire acquire;
...
template<>
void process_auto(NNCountsND<1>& self,
                  const py::array_t<SPosCatalog>& cat,
                  int num_threads) {
    py::gil_scoped_acquire acquire;
...
template<>
void process_auto(NNCountsND<3>& self,
                  const py::array_t<PosCatalog>& cat,
                  bool use_true,
                  int num_threads) {
    py::gil_scoped_acquire acquire;
    using NNType = NNCountsND<3>;
...
void process_cross(NNCountsND<3>& self,
                   const py::array_t<SPosCatalog>& cat1,
                   const py::array_t<SPosCatalog>& cat2,
                   int num_threads) {
    py::gil_scoped_acquire acquire;
    using NNType = NNCountsND<3>;
...
template<>
void process_cross(NNCountsND<2>& self,
                   const py::array_t<SPosCatalog>& cat1,
                   const py::array_t<SPosCatalog>& cat2,
                   int num_threads) {
    py::gil_scoped_acquire acquire;
...
template<>
void process_cross(NNCountsND<1>& self,
                   const py::array_t<SPosCatalog>& cat1,
                   const py::array_t<SPosCatalog>& cat2,
                   int num_threads) {
    py::gil_scoped_acquire acquire;
...
template<>
void process_cross(NNCountsND<3>& self,
                   const py::array_t<PosCatalog>& cat1,
                   const py::array_t<PosCatalog>& cat2,
                   bool use_true,
                   int num_threads) {
    py::gil_scoped_acquire acquire;
...
template<std::size_t N>
void process(NNCountsND<N>& self,
             const py::array_t<SPosCatalog>& cat,
             int num_threads = OMP_NUM_THREADS) {
    ...
    cls.def(py::pickle(
            ...
            [](py::tuple t) { // __setstate__
              if (t.size() != 5 && t.size() != 6) {
                  throw std::runtime_error("Invalid state with "
...
static void declareSPos(py::module& mod) {
    ...
    cls.def("distance_vector",
            [](const SPos& self, const SPos& other) {
              return mkarray_from_array(self.distance_vector(other));
            },
            "Get the Cartesian distance vector between self and other",
            "other"_a);
    ...
    cls.def("check_box",
            py::overload_cast<
                    const SPos&,
                    const BinSpecifier&>(&SPos::check_box, py::const_),
    ...
    cls.def("check_shell",
            py::overload_cast<
                    const SPos&,
                    const BinSpecifier&>(&SPos::check_shell, py::const_),
    ...
    cls.def("check_limits",
            py::overload_cast<
                    const SPos&,
                    const BinSpecifier&,
                    const BinSpecifier&>(&SPos::check_limits, py::const_),
...
static void declarePos(py::module& mod) {
    ...
    cls.def("distance_magnitude",
            ...
            "use_true"_a);
    cls.def("check_box",
            py::overload_cast<const SPos&, double>(&Pos::check_box, py::const_),
            ...
            "max"_a);
    cls.def("check_box",
            py::overload_cast<const SPos&, const BinSpecifier&>(&Pos::check_box,
                                                                py::const_),
    ...
    cls.def("check_shell",
            py::overload_cast<
                    const SPos&,
                    const BinSpecifier&>(&Pos::check_shell, py::const_),
    ...
    cls.def("check_limits",
            py::overload_cast<
                    const SPos&,
                    const BinSpecifier&,
                    const BinSpecifier&>(&Pos::check_limits, py::const_),
...
static py::class_<NNCountsND<N>> declareNNCountsND(py::module& mod) {
    ...
    cls.def("get_1d_index",
            py::overload_cast<const std::array<int, N>&>(&Class::get_1d_indexer,
                                                         py::const_),
    ...
    cls.def_property_readonly("counts",
                              [](const Class& self) {
    ...
    cls.def(py::pickle(
            ...
            [](py::tuple t) { // __setstate__
              ...
              if (t.size() != 3) {
                  ...
                  // on previous version
                  if (t.size() != 2 + N) {
                      throw std::length_error("Invalid state of "
                                              + std::to_string(t.size())
                                              + " elements");
                  }
                  for (std::size_t i = 0; i < N; i++) {
...
void process_separation(ExpectedNNCountsND<1>& self,
...
declareExpectedNNCountsND(py::module& mod) {
    ...
    cls.def_property_readonly("mean",
                              [](const Class& self) {
                                return mkarray_from_vec(self.mean(),
                                                        self.mean_shape_vec());
                              },
                              "The mean counts of the NNCountsND objects");
    cls.def_property_readonly("cov",
                              [](const Class& self) {
                                return mkarray_from_vec(self.cov(),
                                                        self.cov_shape_vec());
                              },
    ...
    cls.def("append_real",
            ...
            ("Append a realization as a single NNCounts"
             + std::to_string(N)
             + "D object").c_str(),
            "other"_a);
    cls.def("append_real",
    ...
    cls.def("append_real",
            ...
            ("Append realizations from each ExpectedNNCounts"
             + std::to_string(N)
             + "D object in a list").c_str(),
            "realizations"_a);
    cls.def(py::pickle(
            [](const Class& c) { // __getstate__
              return py::make_tuple(c.bin_info(),
                                    c.nn_list(),
                                    c.n_real(),
                                    c.n_tot());
            },
...
py::class_<CorrFuncND<N>> declareCorrFuncND(py::module& mod) {
    ...
    using BSType = std::array<BinSpecifier, N>;
    using NNType = NNCountsND<N>;
    py::class_<CorrFuncND<N>>
            cls(mod,
                Class::class_name.c_str(),
                ("Correlation function in " + std::to_string(N) + " dimensions")
                        .c_str());
    cls.def(py::init<>(), "Empty constructor");
    ...
    cls.def("calculate_xi",
            [](const Class& self) {
              return mkarray_from_vec(self.calculate_xi(),
                                      self.shape_vec());
            },
...
py::class_<ExpectedCorrFuncND<N>> declareExpectedCorrFuncND(py::module& mod) {
    ...
    py::class_<ExpectedCorrFuncND<N>>
            cls(mod,
                Class::class_name.c_str(),
    ...
    cls.def("calculate_xi_cov",
            [](const Class& self) {
              return mkarray_from_vec(self.calculate_xi_cov(),
                                      self.cov_shape());
            },
...
static void declare3D(py::module& mod) {
    ...
    nncls.def("get_1d_index",
              py::vectorize(py::overload_cast<
                      int,
                      int,
                      int>(&NNClass::get_1d_indexer, py::const_)),
    ...
    enncls.def("process_separation",
               [](ENNClass& self,
                  ...
                  const py::array_t<double> rl,
                  const py::array_t<double> zb,
                  bool new_real = false,
                  int num_threads = OMP_NUM_THREADS) {
...
static void declare2D(py::module& mod) {
    ...
    ecfcls.def_property_readonly("mean_shape",
                                 [](const ECFClass& self) {
                                   auto
                                           shape_vec = self.mean_shape();
                                   return py::make_tuple(shape_vec.at(0),
                                                         shape_vec.at(1));
                                 });
    ecfcls.def_property_readonly("cov_shape",
                                 [](const ECFClass& self) {
                                   auto
                                           shape_vec = self.cov_shape();
                                   return py::make_tuple(shape_vec.at(0),
                                                         shape_vec.at(1),
                                                         shape_vec.at(2),
                                                         shape_vec.at(3));
