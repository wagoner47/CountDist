...
template<std::size_t N>
py::array_t<norm_type, 0> convert_normed_counts_vec(const NNCountsND <N>& nn) {
    std::cout << "In 'convert_normed_counts_vec'" << std::endl;
...
static void bind_get_1d_indices(py::module& mod) {
    mod.def(("get_1d_index_from_" + std::to_string(N) + "d").c_str(),
            py::overload_cast<const std::array<int, N>&,
                              const std::array <BinSpecifier, N>&>(
                    &get_1d_indexer_from_nd < N > ),
    ...
    mod.def(("get_1d_index_from_" + std::to_string(N) + "d").c_str(),
            py::overload_cast<const py::array_t<int>&,
                              const std::array <BinSpecifier, N>&>(&get_1d_index_from_nd<
                    N>),
...
py::array_t <SepDType>
get_separations_arr(const py::array_t <SPosCatalog>& cat1,
                    const py::array_t <SPosCatalog>& cat2,
...
py::array_t <TOSepDType>
get_separations_arr(const py::array_t <PosCatalog>& cat1,
                    const py::array_t <PosCatalog>& cat2,
                    const BinSpecifier& rp_bins, const BinSpecifier& rl_bins,
                    bool is_auto, int num_threads = OMP_NUM_THREADS) {
    auto pos1 = convert_catalog(cat1);
...
template<std::size_t N>
void process(NNCountsND <N>& self, const py::array_t <PosCatalog>& cat,
             bool use_true, int num_threads = OMP_NUM_THREADS) {
...
static py::class_ <NNCountsND<N>> declareNNCountsND(py::module& mod) {
    ...
    cls.def("process",
            py::overload_cast<Class&, const py::array_t <PosCatalog>&,
                              const py::array_t <PosCatalog>&, bool, bool, int>(
                    &process<N>),
            py::call_guard<py::gil_scoped_release>(),
    ...
    cls.def_property_readonly("counts",
                              ...
                              ("Counts of pairs in " + std::to_string(N)
                               + "D bins").c_str(),
...
py::class_ <CorrFuncND<N>> declareCorrFuncND(py::module& mod) {
    ...
    cls.def("calculate_xi_numerator",
            ...
            "Calculate the numerator of the correlation function for the given estimator",
